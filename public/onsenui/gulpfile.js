...
gulp.task('cssmin', ['generate-preview'], () => {
  return gulp.src(prefix + 'onsenui-css-components.css')
    .pipe($.cssmin())
...
gulp.task('cssnext', ['stylelint'], () => {
  ...
  return gulp.src('src/onsenui-css-components.css')
    .pipe($.plumber())
...
gulp.task('generate-preview', ['cssnext'], () => {
  ...
  const css = fs.readFileSync(prefix + 'onsenui-css-components.css', 'utf-8');
